...
結果: この仮説は未検証のまま (results/ に有効な出力なし)
  実行はできるが、有効なデータが取れなかった。理由は測定方法の欠陥である。

    - llm-token-limit のカウンタは counter-key ごとに APIM 側で保持され、
      期間が明けるまでリセットできない。
    - そのため上限値を変えて連続実行すると、前の条件の消費が残った状態で
      次の条件が始まる。逆に期間明けを待つと、待機中に別の要因が混ざる。
    - 実際、測定中にカウンタが複数回リセットされ、どの区間の消費が
      どの条件に属するのか切り分けられなくなった。

  正しく測るには、条件ごとに別の counter-key (別サブスクリプション) を用意し、
  各条件を1回ずつ、期間をまたがずに実行する必要がある。

  仮説そのものは quota_overshoot.py の実測 (超過量 611 ≒ 2件 x 300トークン) と
  整合するが、「上限値を変えても超過量の絶対値が変わらない」ことは
  直接確かめられていない。docs/verification-summary.md にも未検証と明記している。
